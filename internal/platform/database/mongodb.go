...
func (cm *ConnectionManager) CreateIndexes(ctx context.Context) error {
	patientsCollection := cm.GetCollection("patients")
	...
	indexes := []mongo.IndexModel{
		...
		},
		{
			...
			Options: options.Index().SetName("id_1").SetUnique(true),
		},
	}

	_, err := patientsCollection.Indexes().CreateMany(ctx, indexes)
	if err != nil {
		cm.logger.Error("Failed to create MongoDB indexes", zap.Error(err))
		return fmt.Errorf("failed to create indexes: %w", err)
	}

	cm.logger.Info("Successfully created MongoDB indexes")
	return nil
}
